...
- **时间为 AI 权威**：`timePassedMonths` 由 AI 返回——返回多少推进多少（钳制 0~12），未返回 / 返回 0 / 回合失败一律不流逝；代码不再做叙事时间解析（曾用正则折算「数日/半月/数月」但中文时间表述歧义太多，已废弃）。时间线只按整月推进，不足一年的月数余数跨回合累积（`flags.ageMonths`），攒满 12 个月折算 1 年，保证「入道第几年」与年龄/寿元始终同步
...
- **无 Key 降级**：未配置 Key 时系统指令（修炼/突破/战斗/坊市等）仍可玩（代码结算，数值一致）；自由行动与叙事演绎需配置叙事引擎
- **回合卡自检**：每张回合卡显示「⌛ 流逝 X 月 / 未流逝」与「【数值变化】」，时间与状态来源一目了然

## 背景图（水墨修仙风）

`public/bg/` 下 10 张背景（**WebP 优先，PNG → SVG 兜底**；`npm run opt-bg` 生成 WebP 压缩版）。**用 AI 绘图工具替换背景时**，提示词见 [BACKGROUND-PROMPTS.md](BACKGROUND-PROMPTS.md)——按文件名（`title.png`/`qingyu.png`…）保存到 `public/bg/` 即生效：
...
## 异常状态与 deltas（AI 同步 · 最终版）

- 伤势 `injury`：AI 用中文名（轻伤/重伤/垂死/内伤/中毒/心魔缠身）或 `null` 清除，自动映射内部 id；状态卡显示中文名
- 异常 `status`：字符串数组（`["中毒"]`）或单个字符串均可，`[]` 清除
- 心境 `mood`：0.5 / 1.0 / 1.2
- 好感 `affinity`（NPC 名，0~100）/ 背包 `bag`（物品名，正加负减）/ 悟道 `enlightenment` / 技艺 `technique`
- 所在地 `location`：AI 写玩家移动后必须同步（五洲名/宗门/秘境名），状态卡与快照联动
- 主线 `mainQuest`：AI 推进主线时更新状态卡提示
- 系统指令回合：AI 的 deltas 以「状态模式」应用——数值与六维跳过（已由代码结算，防双加），上述非数值字段全部采纳；自由行动回合全量应用
