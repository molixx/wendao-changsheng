...
timePassedMonths（0~12，可缺省）：**以 summary 中写的时间描述为准**（前端会解析 summary），本字段仅作补充，不必非填。瞬时事件 0、数日 0.3、半月 0.5、一月 1、数月 2~6、半年 6、一年 12。
...
3. 给出本回合流逝月数 timePassedMonths（0~12，可缺省）：**以 summary 中写的时间描述为准**，本字段仅作补充不必非填（瞬时事件 0、数日 0.3、半月 0.5、一月 1、数月 2~6、半年 6、一年 12）。
...
5. 给出 summary（**必填**）：20~40 字一句话概述本回合剧情（发生什么/结果如何），**必须包含本回合时间流逝**（如「闭关三月修为+30」「瞬时空闲未流逝时间」），禁止截取 narrative 开头。
