...
timePassedMonths（0~12，可缺省）：本回合流逝的月数。**你返回多少，游戏时间就推进多少；返回 0 或缺省则不流逝**。请按叙事中实际的时间流逝填写：瞬时事件（说话/闲逛/思索）→ 0；「数日」→ 0（不足一月）；「半月」→ 0.5；「一月」→ 1；「数月」→ 2~6；「半年」→ 6；「一年」→ 12。不要无谓 +1。
...
3. 给出本回合流逝月数 timePassedMonths（0~12，可缺省）：**你返回多少，游戏时间就推进多少；返回 0 或缺省则不流逝**。按叙事实际时间流逝填写（瞬时事件 0、数日 0、半月 0.5、一月 1、数月 2~6、一年 12），不要无谓 +1。
4. 状态变化用 deltas 声明（可选）：系统已结算数值，你**只能**给状态类字段——{"injury": "轻伤"}（中文名：轻伤/重伤/垂死/内伤/中毒/心魔缠身，null 清除）、{"status": ["中毒"]}（字符串数组，[] 清除）、{"mood": 1.2}（0.5/1.0/1.2）。不要给 hp/mp/cult 等数值字段。
只输出一个 JSON 对象：{"narrative": "...", "options": [{"text": "...", "tag": "平和"}], "timePassedMonths": 0, "deltas": {}}，narrative 必须为纯中文文字，不要输出 JSON 之外的任何内容。`,
      },
      ...history,
