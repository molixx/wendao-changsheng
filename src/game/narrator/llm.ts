...
timePassedMonths（0~12）：本回合流逝的月数，**你是时间的唯一权威**。规则：
1. 叙事中完全没有写时间流逝（说话/闲逛/思索等瞬时事件）→ 必须返回 0；
2. 叙事中写了时间流逝 → 返回与叙事文字相符的值：「数日/几日」→ 0（不足一月按 0）、「半月」→ 0.5、「一月」→ 1、「数月」→ 2~6、「半年」→ 6、「一年」→ 12；
3. 铁律：返回值必须与叙事文字相符——叙事写「数日」却返回 12、叙事没写时间却返回 1，都是严重错误；
4. 绝不要因为「每回合」就无谓 +1，更不要把 1 或 12 当默认值。
...
3. 给出本回合流逝月数 timePassedMonths（0~12）：**你是时间的权威**——叙事没写时间流逝就返回 0；写了就返回与文字相符的值（数日→0、半月→0.5、一月→1、数月→2~6、一年→12）。绝不要无谓 +1，更不要把 1 或 12 当默认值。
